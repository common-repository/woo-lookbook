...
class WOO_F_LOOKBOOK_Admin_Product {
	...
	public function product_scripts() {
		...
		if ( 'product' == get_post_type() && 'product' == $screen->id ) {
			...
			wp_enqueue_style( 'woo-lookbook-admin-product', WOO_F_LOOKBOOK_CSS . 'woo-lookbook-admin-product' . $this->suffix . '.css', array(), WOO_F_LOOKBOOK_VERSION );

			wp_enqueue_media();
			wp_enqueue_script( 'select2', WOO_F_LOOKBOOK_JS . 'select2.min.js', array( 'jquery' ), '4.0.5', false );
			wp_enqueue_script( 'woo-lookbook-admin-product', WOO_F_LOOKBOOK_JS . 'woo-lookbook-admin-product' . $this->suffix . '.js', array( 'jquery' ), WOO_F_LOOKBOOK_VERSION, true );
	...
	public function save_metabox( $post_id, $post ) {
		...
		$nonce_name   = isset( $_POST['_wlb_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_wlb_nonce'] ) ) : '';
