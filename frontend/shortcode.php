...
class WOO_F_LOOKBOOK_Frontend_Shortcode {
	...
	public function shortcode_scripts() {
		...
		//exe
		wp_register_script( 'woo-lookbook', WOO_F_LOOKBOOK_JS . 'woo-lookbook'. $this->suffix .'.js', array( 'jquery' ), true, true );
...
//		add_action( 'wp_footer', array( $this, 'overlay' ) );
	...
	public function register_shortcode_instagram( $atts ) {
		...
		ob_start(); ?>
        <div class="woo-lookbook <?php echo 'carousel' == $atts['style'] ? 'wlb-lookbook-carousel' : 'wlb-lookbook-gallery' ?>"
                                ...
                                <div class="wlb-lookbook-instagram-item-inner">
									...
									if ( is_array( $products ) && count( $products ) ) {
										foreach ( $products as $k => $product ) {
											...
											echo wp_kses_post( $this->get_node( $product, $pos_x[ $k ], $pos_y[ $k ] ) );
										}
									} ?>
	 ...
	 * @param $atts
	 */
	public function register_shortcode_slide(
		$atts
	) {
		$this->enqueue_scripts();
		$this->run_shortcode = true;
