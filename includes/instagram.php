	...
	class VillaTheme_Instagram {
		...
		public function save_fb_info() {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$link_call_back = isset( $_GET['page'] ) && 'woocommerce-lookbook-settings' == $_GET['page'] ? add_query_arg( array( 'page' => 'woocommerce-lookbook-settings#/instagram' ), $link_call_back ) : $link_call_back;
		...
		public function get() {
			...
			if ( ! $access_token ) {
				if ( wp_doing_ajax() ) {
					wp_send_json_error( esc_html__( 'Access token is not exist', 'woo-lookbook' ) );
				}
			...
			if ( ! $id ) {
				if ( wp_doing_ajax() ) {
					wp_send_json_error( esc_html__( 'Instagram id is not exist', 'woo-lookbook' ) );
				}
			...
			if ( $this->check_token_live( $access_token ) ) {
				...
				if ( wp_doing_ajax() ) {
					wp_send_json_error( esc_html__( 'Access token is expired', 'woo-lookbook' ) );
				}
			}
		}
		...
		public function fb_get( $id, $access_token, $fields, $limit = '', $graph = 'node', $type = '' ) {
			...
			try {
				...
				echo esc_html__( 'Graph returned an error: ', 'woo-lookbook' ) . esc_html( $e->getMessage() );
				exit;
			} catch ( Facebook\Exceptions\FacebookSDKException $e ) {
				echo esc_html__( 'Facebook SDK returned an error: ', 'woo-lookbook' ) . esc_html( $e->getMessage() );
				exit;
			}

			$graph_response = 'node' == $graph ? $response->getGraphNode()->asArray() : $response->getGraphEdge()->asArray();
...
//				exit;\
		...
		public function get_link_login( $link_callback, $permissions = '' ) {
			...
			if ( empty( $link_callback ) ) {
				return array(
					'status' => false,
					'msg'    => esc_html__( 'Link Callback not found!', 'woo-lookbook' )
				);
			}
			...
			$login_url = $helper->getLoginUrl( $link_callback, $permissions );
		...
		public function get_token( $link_call_back ) {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( isset( $_GET['state'] ) ) {
				$helper->getPersistentDataHandler()->set( 'state', sanitize_text_field( wp_unslash( $_GET['state'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			}
			try {
				$access_token = $helper->getAccessToken( $link_call_back );
		...
		protected function upload_image( $url, $desc = '' ) {
			...
			preg_match( '/[^\?]+\.(jpg|JPG|jpeg|JPEG|jpe|JPE|gif|GIF|png|PNG|webp|WEBP)/', $thumb_url, $matches );
			if ( empty( $matches ) ) {
				return false;
			}
